def decoder_a(in_dict, register):
    data_a = register[0][in_dict["rA"]]
    data_b = register[0][in_dict["rB"]]
    data_c = in_dict["const"]
    data_s = register[0][0xFE]
    op = in_dict["op"]
    tail = in_dict["tail"]
    ...
    if tail in (0x00, 0xFF):
        if op == 0x00: # halt
            ...
            t = register[0][0x100]
            register[0][0x100] = data_c + data_b
            data_c = t
        elif op == 0x71: # ret
            pass
        else:
    ...
    return {"data_a": data_a, "data_b": data_b, "data_c": data_c, "data_s": data_s, "status": status, "simd": 0,
